Shader "Sprites/Outline (Destructible 2D)"
{
	Properties
	{
		...
		Pass
		{
			CGPROGRAM
				...
				void Frag(v2f i, out float4 o:COLOR0)
				{
					...
					o.a = saturate(_AlphaThreshold + (alphaTex.a - (1.0f - _AlphaThreshold)) * _Sharpness);
					
					// Apply outline with custom threshold
					float outline = saturate(_OutlineThreshold + (alphaTex.a - (1.0f - _OutlineThreshold)) * _Sharpness);
